...
public class SystemParser extends AbstractParser {
	...
	@Command
	public String cluster(String[] parts) {
		Assert.checkParamsCount(parts.length == 1);
		if (!clusterManager.isEnable()) {
			return Constants.CLUSTER_NOT_OPEN;
		}
		return "MasterNodes: " + clusterManager.getMasterNodes() + "\n" +
				"SlaverNodes: " + clusterManager.getSlaveNodes();
	}
